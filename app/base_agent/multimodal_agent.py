...
class NoTokensAvailableError(Exception):
    ...
    def __init__(self, message="User does not have sufficient tokens available. Please recharge."):
        self.message = message
        super().__init__(self.message)

class Reviewer():
  def __init__(self, review_prompt:str=None, review_function=None):    
    self.review_prompt = review_prompt or "Please review the conversation and provide feedback."
...
class MultiModalAgent():
    def __init__(self, name:str = None, system_prompt:str=None, reviewer:Reviewer=None, connector:Connector=None, reasoning=False,
                 update_token_callback=None, check_token_callback=None):
        if name is None:
    ...
    def execute_user_ask(self, user_input=None, chat_history=None, base64image=None, temperature=0.7, filters=None, model=None, json_response=False, reasoning=None, tools=None) -> tuple:
        ...
        """
        response, chat_history = self._get_response(user_input=user_input, chat_history=chat_history, base64image=base64image, temperature=temperature, filters=filters, model=model,
                                                    json_response=json_response, reasoning=reasoning, tools=tools)
        ## As long as the agent is asking to make tool calls, lets do that.
        while len(response) > 1 or (len(response)>0 and response[0]["type"] == "toolcall"):
            for single_response in response:
                if single_response["type"] == "toolcall":
                    tool_response = self.connector.make_tool_calls(single_response["value"])
                    logger.debug(f"Printing tool response : {tool_response}")
                    chat_history = self.connector.update_chat_history_with_toolcall_response(tool_response, chat_history)
            response, chat_history = self._get_response(chat_history=chat_history, temperature=temperature, json_response=json_response,
                                                        reasoning=reasoning, filters=filters, model=model, tools=tools)
        if self.reviewer is not None:
            reviewer_msg, reviewer_img = self.reviewer.get_message(response)
            response, chat_history = self._get_response(user_input=reviewer_msg, chat_history=chat_history, base64image=reviewer_img, temperature=temperature, filters=filters, model=model,
                                                        json_response=json_response, reasoning=reasoning, tools=tools)
            
        return response[0]["value"], chat_history
    ...
    @retry(stop_max_attempt_number=3, wait_fixed=1000, wait_exponential_multiplier=2000, retry_on_exception=should_retry_exception)
    def _get_response(self, chat_history=None, user_input=None, base64image=None, temperature=0.7, filters=None, model=None, 
        ...
        system_message = self.connector.get_system_message(self.system_prompt, self.name)
        ...
        if user_input is not None or base64image is not None:
            created_message = self.connector.create_message(base64_image=base64image, text=user_input)
            messages = messages + created_message
        
        if model is None:
            result = self.connector.get_response(chat_history=messages,system_message=system_message, temperature=temperature, 
                                                 json_response=json_response, reasoning=reasoning, tools=tools)
        else:
            result = self.connector.get_response(chat_history=messages,system_message=system_message, temperature=temperature, model=model,
                                                 json_response=json_response, reasoning=reasoning, tools=tools)
