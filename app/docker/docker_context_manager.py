...
class DockerContextManager:
    ...
    @staticmethod
    async def get_vm_info_for_user_workspace(username: str, workspace_name: str) -> Optional[dict]:
        """
        ...
        if workspace and workspace.vm_config and workspace.vm_config.private_ip:
            return {'ip': workspace.vm_config.private_ip}
        raise ValueError(f"Workspace {workspace_name} for user {username} does not have valid VM info or is not allocated.")
    ...
    @staticmethod
    async def set_context_for_user_workspace(username: str, workspace_name: str, user: str = 'azureuser'):
        """
        ...
        host = vm_info.get('ip') or vm_info.get('hostname')
        private_ip = vm_info.get('ip')
        ...
        if result.returncode == 0:
            ...
            logger.info(f"Docker context '{context_name}' already exists.")
            return context_name, private_ip
